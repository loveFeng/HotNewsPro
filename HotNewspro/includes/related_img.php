...
<ul>
	...
	while (have_posts()) : the_post();?>
		...
		<?php if ( get_post_meta($post->ID, 'thumbnail', true) ) : ?>
			...
			<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title(); ?>"><img src="<?php echo $image; ?>" alt="<?php the_title(); ?>"/></a>
			<?php else: ?>
			<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title(); ?>"><img src="<?php bloginfo('template_directory'); ?>/images/random/r_m<?php echo rand(1,3)?>.jpg" alt="<?php the_title(); ?>" /></a>
			<?php endif; ?>
		</div>
	<?php endwhile;endif; ?>
</ul>
